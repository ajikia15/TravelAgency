<script>
	...
	const getWeather = async () => {
		...
		data.daily.forEach((day) => {
			...
			day.dateString = date.toLocaleDateString(undefined, {
				month: 'long',
				day: 'numeric'
			});
		});
...
<div class="flex flex-col items-center justify-center text-gray-700">
	<div class="w-full rounded-lg bg-white px-10 pt-6 shadow-md">
		...
		{#if data.lat}
			<div class="flex justify-between">
				<div class="flex flex-col">
					...
					<span class="mt-1 font-semibold text-gray-500">
						{Location} Today, {data.daily[0].dateString}
					</span>
				</div>
			...
			<div class="mx-2 mt-3 flex justify-between">
			...
			<Dialog>
				<DialogTrigger>
					<button type="button" class="text-muted-foreground line-clamp-1 pb-5 pt-2 hover:underline"
						>8 Day Forecast &rarr;</button>
				</DialogTrigger>
				<DialogContent>
					<DialogHeader>
